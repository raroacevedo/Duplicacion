...
def import_course_content(driver, course):
    ...
    update_excel(path, course["Maestro"], brightspace_id)

def update_excel(file_path, maestro, brightspace_id):
    
    """Busca el maestro en Excel y actualiza la columna 'Oferta de curso - Id grupo Brightspace' sin perder formatos."""
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=1):
        for cell in row:
            if str(cell.value).strip() == str(maestro).strip():  
                brightspace_col = 6  
                ws.cell(row=cell.row, column=brightspace_col, value=int(brightspace_id))
                print(f"\nID {brightspace_id} agregado al excel correctamente")
                found = True
                break

    if not found:
        print(f"\nMaestro {maestro} no encontrado en el archivo Excel.")
    ...
    """Busca el maestro en Excel y actualiza la columna 'Oferta de curso - Id grupo Brightspace' sin perder formatos."""
    """wb = openpyxl.load_workbook(file_path)  # Cargar el archivo
    ws = wb.active  # Seleccionar la hoja activa
    
     # Crear un estilo predeterminado si no existe
    if "default" not in wb.named_styles:
        default_style = NamedStyle(name="default")
        wb.add_named_style(default_style)

    # Buscar la fila con el maestro
    found = False
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=1):
        for cell in row:
            if str(cell.value).strip() == str(maestro).strip():  # Comparar con la columna 'Maestro'
                brightspace_col = 6  # Columna donde queremos escribir (Oferta de curso - Id grupo Brightspace)
                ws.cell(row=cell.row, column=brightspace_col, value=int(brightspace_id))  # Guardar como número
                print(f"\nID {brightspace_id} agregado al excel correctamente")
                found = True
                break
    ...
    wb.save(file_path)  # Guardar cambios correctamente
